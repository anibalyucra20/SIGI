...
<style>
  @media (min-width: 992px) { .five-cols .form-col { flex: 0 0 20%; max-width: 20%; } }

  /* Tarjetas uniformes y botón “Ver” alineado abajo */
  .book-card{ height:100%; display:flex; flex-direction:column; }
  .book-cover-wrap{
    aspect-ratio:3/4; background:#f6f7f9; display:flex; align-items:center; justify-content:center; overflow:hidden;
...
(() => {
  ...
  $('#id_programa_estudios option').each(function(){
    const v = this.value; const t = this.textContent.trim();
  ...
  function ensurePlanesForProg(idProg){
    ...
    return $.getJSON(`${BASE}/sigi/planes/porPrograma/${encodeURIComponent(idProg)}`)
      .then(arr => { dict.planesByProg.set(idProg, arr); arr.forEach(pl => dict.planName.set(String(pl.id), pl.nombre)); });
  }
  function ensureModulosForPlan(idPlan){
    ...
    return $.getJSON(`${BASE}/sigi/moduloFormativo/porPlan/${encodeURIComponent(idPlan)}`)
      .then(arr => { dict.modsByPlan.set(idPlan, arr); arr.forEach(m => dict.modName.set(String(m.id), m.descripcion)); });
  }
  function ensureSemestresForModulo(idMod){
    ...
    return $.getJSON(`${BASE}/sigi/semestre/porModulo/${encodeURIComponent(idMod)}`)
      .then(arr => { dict.semsByMod.set(idMod, arr); arr.forEach(s => dict.semName.set(String(s.id), s.descripcion)); });
  }
  function ensureUDsForSemestre(idSem){
    ...
    return $.getJSON(`${BASE}/sigi/unidadDidactica/porSemestre/${encodeURIComponent(idSem)}`)
      .then(arr => { dict.udsBySem.set(idSem, arr); arr.forEach(u => dict.udName.set(String(u.id), u.nombre)); });
  }
  function ensureDictionaries(rows){
    const needProgForPlans = new Set();
    rows.forEach(r=>{ const v=r.vinculo||{}; if(v.id_plan && !dict.planName.has(String(v.id_plan)) && v.id_programa_estudio) needProgForPlans.add(String(v.id_programa_estudio)); });
    return Promise.all([...needProgForPlans].map(ensurePlanesForProg)).then(()=>{
      const needPlanForMods = new Set();
      rows.forEach(r=>{ const v=r.vinculo||{}; if(v.id_modulo_formativo && !dict.modName.has(String(v.id_modulo_formativo)) && v.id_plan) needPlanForMods.add(String(v.id_plan)); });
      return Promise.all([...needPlanForMods].map(ensureModulosForPlan));
    }).then(()=>{
      const needModForSems = new Set();
      rows.forEach(r=>{ const v=r.vinculo||{}; if(v.id_semestre && !dict.semName.has(String(v.id_semestre)) && v.id_modulo_formativo) needModForSems.add(String(v.id_modulo_formativo)); });
      return Promise.all([...needModForSems].map(ensureSemestresForModulo));
    }).then(()=>{
      const needSemForUDs = new Set();
      rows.forEach(r=>{ const v=r.vinculo||{}; if(v.id_unidad_didactica && !dict.udName.has(String(v.id_unidad_didactica)) && v.id_semestre) needSemForUDs.add(String(v.id_semestre)); });
  ...
  function buildPager(total, page, per){
    ...
    function li(lbl, target, disabled=false, active=false){
      const cls = ['page-item']; if(disabled) cls.push('disabled'); if(active) cls.push('active');
    ...
    $p.find('a[data-page]').on('click', function(e){ e.preventDefault();
  ...
  $('#id_programa_estudios').on('change', function(){
    ...
    if(idProg){
      ...
      }).always(()=>{ state.page=1; loadPage(); });
    }else{ state.page=1; loadPage(); }
  });
  ...
  let tmr; const deb = fn => { clearTimeout(tmr); tmr=setTimeout(fn,300); };
  $('#f_titulo').on('input', ()=>deb(()=>{ rebuildSearch(); state.page=1; loadPage(); }));

  $('#btnApplyFilters').on('click', ()=>{ rebuildSearch(); state.page=1; loadPage(); });
