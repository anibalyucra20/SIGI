...
require_once __DIR__ . '/../../../app/models/Tutoria/Programacion.php';
...
use App\Models\Tutoria\Programacion;
...
class ProgramacionController extends Controller
{
    ...
    public function editar($id){
        if (\Core\Auth::esAdminTutoria()):
            ...
            if (!$data) {
                $_SESSION['flash_error'] = "No se encontró la programación de tutoría";
                header('Location: ' . BASE_URL . '/tutoria/programacion');
                exit;
            }
        endif;
        $this->view('tutoria/programacion/editar', [
            'data' => $data,
            'docentes' => $docentes,
            'isEdit'    => true,
            'module' => 'tutoria',
            'pageTitle' => 'Editar Programación de Tutoría',
        ]);
        exit;
    }
    ...
    public function guardar()
    {
        ...
        if (\Core\Auth::esAdminTutoria() && $periodo_vigente):
            ...
            if (!empty($errores)) {
                ...
                header('Location: ' . BASE_URL . '/tutoria/programacion/' . $link . '/' . ($_POST['id'] ?? ''));
                exit;
            }
            ...
            try {
                ...
                $_SESSION['flash_success'] .= "Programación de tutoría registrada exitosamente.";
                header('Location: ' . BASE_URL . '/tutoria/programacion');
                exit;
            } catch (\Throwable $e) {
                $_SESSION['flash_error'] .= "No se pudo registrar: " . $e->getMessage();
                header('Location: ' . BASE_URL . '/tutoria/programacion/nuevo');
                exit;
            }
        else:
    ...
    public function eliminar($id)
    {
        if (\Core\Auth::esAdminTutoria()):
            try {
                ...
                $_SESSION['flash_success'] = "Programación de tutoría eliminada exitosamente.";
            } catch (\Throwable $e) {
