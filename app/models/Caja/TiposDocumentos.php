...
class TiposDocumentos extends Model
{
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM caja_tipo_documento ORDER BY estado DESC");
        $stmt->execute();
    ...
    public function getPaginated($filters, $length, $start, $orderCol, $orderDir)
    {
        ...
        // Conteo total
        $sqlTotal = "SELECT COUNT(*) FROM caja_tipo_documento 
            $sqlWhere";
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM caja_tipo_documento WHERE id = ?");
        $stmt->execute([$id]);
    ...
    {
        if (!empty($data['id'])) {
            $sql = "UPDATE caja_tipo_documento SET 
                        codigo = :codigo, 
            ...
            $sql = "INSERT INTO caja_tipo_documento 
                        (codigo, descripcion) 
                    VALUES 
                        (:codigo, :descripcion)";
            $params = [
    ...
    {
        $stmt = self::$db->prepare("DELETE FROM caja_tipo_documento WHERE id=?");
        $stmt->execute([$id]);
    }
    public function validar($data, $isEdit = false, $id = null)
    {
        ...
        if (!$isEdit || ($isEdit && $id)) {
            // codigo único
            $q = "SELECT COUNT(*) FROM caja_tipo_documento WHERE codigo = :codigo";
