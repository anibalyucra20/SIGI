...
class MediosPago extends Model
{
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM caja_medios_pago ORDER BY estado DESC");
        $stmt->execute();
    ...
    public function getPaginated($filters, $length, $start, $orderCol, $orderDir)
    {
        ...
        // Conteo total
        $sqlTotal = "SELECT COUNT(*) FROM caja_medios_pago 
            $sqlWhere";
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM caja_medios_pago WHERE id = ?");
        $stmt->execute([$id]);
    ...
    {
        if (!empty($data['id'])) {
            $sql = "UPDATE caja_medios_pago SET 
                        codigo = :codigo, 
            ...
            $sql = "INSERT INTO caja_medios_pago 
                        (codigo, descripcion) 
                    VALUES 
                        (:codigo, :descripcion)";
            $params = [
    ...
    public function validar($data, $isEdit = false, $id = null)
    {
        ...
        if (!$isEdit || ($isEdit && $id)) {
            // DNI único
            $q = "SELECT COUNT(*) FROM caja_medios_pago WHERE codigo = :codigo";
    ...
    {
        $stmt = self::$db->prepare("DELETE FROM caja_medios_pago WHERE id=?");
        $stmt->execute([$id]);
    }
}
