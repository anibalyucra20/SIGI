...
class RubrosEgresosContables extends Model
{
    public function listar()
    {
        $stmt = self::$db->prepare("SELECT * FROM caja_rubros_egresos_contab ORDER BY estado DESC");
        $stmt->execute();
    ...
    public function getPaginated($filters, $length, $start, $orderCol, $orderDir)
    {
        ...
        $sql = "SELECT *
                FROM caja_rubros_egresos_contab
                $sqlWhere
        ...
        // Conteo total
        $sqlTotal = "SELECT COUNT(*) FROM caja_rubros_egresos_contab 
            $sqlWhere";
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM caja_rubros_egresos_contab WHERE id = ?");
        $stmt->execute([$id]);
    ...
    {
        if (!empty($data['id'])) {
            $sql = "UPDATE caja_rubros_egresos_contab SET 
                        codigo = :codigo, 
            ...
            $sql = "INSERT INTO caja_rubros_egresos_contab 
                        (codigo, descripcion) 
                    VALUES 
                        (:codigo, :descripcion)";
            $params = [
    ...
    {
        $stmt = self::$db->prepare("DELETE FROM caja_rubros_egresos_contab WHERE id=?");
        $stmt->execute([$id]);
    }
    public function validar($data, $isEdit = false, $id = null)
    {
        ...
        if (!$isEdit || ($isEdit && $id)) {
            // DNI único
            $q = "SELECT COUNT(*) FROM caja_rubros_egresos_contab WHERE codigo = :codigo";
