...
class CentroCostos extends Model
{
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM caja_centro_costos ORDER BY estado DESC");
        $stmt->execute();
    ...
    public function getPaginated($filters, $length, $start, $orderCol, $orderDir)
    {
        ...
        // Conteo total
        $sqlTotal = "SELECT COUNT(*) FROM caja_centro_costos 
            $sqlWhere";
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM caja_centro_costos WHERE id = ?");
        $stmt->execute([$id]);
    ...
    {
        if (!empty($data['id'])) {
            $sql = "UPDATE caja_centro_costos SET 
                        codigo = :codigo, 
            ...
            $sql = "INSERT INTO caja_centro_costos 
                        (codigo, descripcion) 
                    VALUES 
                        (:codigo, :descripcion)";
            $params = [
    ...
    public function validar($data, $isEdit = false, $id = null)
    {
        ...
        if (!$isEdit || ($isEdit && $id)) {
            // DNI único
            $q = "SELECT COUNT(*) FROM caja_centro_costos WHERE codigo = :codigo";
    ...
    {
        $stmt = self::$db->prepare("DELETE FROM caja_centro_costos WHERE id=?");
        $stmt->execute([$id]);
    }
}
